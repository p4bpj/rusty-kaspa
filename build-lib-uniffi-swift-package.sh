...
lipo -create target/x86_64-apple-ios/release/libkaspa_wallet_core.a target/aarch64-apple-ios-sim/release/libkaspa_wallet_core.a -output $fat_simulator_lib_dir/libkaspa_wallet_core.a


echo "Generating framework module mapping and FFI bindings"
# NOTE: Convention requires the modulemap be named module.modulemap
cargo run --bin uniffi-bindgen-swift -- target/aarch64-apple-ios/release/libkaspa_wallet_core.a target/uniffi-xcframework-staging --swift-sources --headers --modulemap --module-name KaspaWalletCore --modulemap-filename module.modulemap
mkdir -p swift-package-ios/apple/Sources/UniFFI/
mv target/uniffi-xcframework-staging/*.swift swift-package-ios/apple/Sources/UniFFI/
...
xcodebuild -create-xcframework \
  -library target/aarch64-apple-ios/release/libkaspa_wallet_core.a -headers target/uniffi-xcframework-staging \
  -library target/ios-simulator-fat/release/libkaspa_wallet_core.a -headers target/uniffi-xcframework-staging \
  -output target/ios/libkaspa_wallet_core-rs.xcframework
